...
# Se genera asi redcompartida.com/ajax/imagen/gen.[IDIMAGEN].[IDANUNCIO].[ANCHO].[PROPORCION]
# Ejemplo: redcompartida.com/ajax/imagen/gen.8.64.190.4:3

$iIdImagen = $mxParametro[0];
$iIdAnuncio = $mxParametro[1];
$_GET['width'] = $mxParametro[2];
$_GET['cropratio'] = $mxParametro[3];

if ($iIdAnuncio == 0) {
    $aDatosImagen = $db->getRecord(
            "SELECT md5, extension " .
            "FROM imagen ". 
            "WHERE idImagen = $iIdImagen"
    );
}else{

    $aDatosImagen = $db->getRecord(
            "SELECT md5, nombreOrigin, extension " .
            "FROM anuncioImagen a ". 
            "INNER JOIN imagen i ON (i.idImagen = a.idImagen) ". 
            "WHERE a.idAnuncio = $iIdAnuncio AND a.idImagen = $iIdImagen"
    );
    
}

$sMd5 = $aDatosImagen['md5'];
//$sNombre = $aDatosImagen['nombreOrigin'];
$sExtension = $aDatosImagen['extension'];

$mxImagen = file_get_contents($aConfiguracion['directorios']['imagenes'].'/'.$sMd5);

// width=ancho&amp;cropratio=4:3&amp;image=/url_foto.jpg" 

// = '16:9';
$_GET['image'] = "/".$aConfiguracion['directorios']['imagenes'].'/'.$sMd5;;

//Header( "Content-type: ".$sExtension); 
//echo $mxImagen;
...
// back to Shifting Pixel (either this page or shiftingpixel.com), but donít worry about
// including a big link on each page if you donít want toñone will do just nicely. Feel
